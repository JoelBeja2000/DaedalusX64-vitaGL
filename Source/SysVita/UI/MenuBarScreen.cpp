...
void DrawCanvasEditOverlay() {
	...
	snprintf(legend_str, sizeof(legend_str),
		"[DPAD] Move  |  [▲/✖] Size: %dpx  |  [■] Shape  |  [L1/R1] Zone %d/8  |  [START] Save & Exit",
		gTouchZones[gSelectedTouchZone].radius_w, gSelectedTouchZone + 1);

	draw_list->AddText(ImVec2(20, 516), IM_COL32(255, 255, 255, 255), legend_str);
...
void DrawTouchZonesWindow() {
	...
	if (ImGui::Begin("Touch Zone Coordinate Mapping", &touch_zones_window)) {
		...
		if (ImGui::SliderInt("Center X", &cx, 0, 1920)) tz.center_x = (uint16_t)cx;
		if (ImGui::SliderInt("Center Y", &cy, 0, 1088)) tz.center_y = (uint16_t)cy;
